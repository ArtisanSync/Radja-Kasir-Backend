...
try {
  ...
  if (missingVars.length > 0) {
    console.warn(`Konfigurasi ImageKit tidak lengkap. Yang kurang: ${missingVars.join(', ')}. Fitur Storage akan dinonaktifkan.`);
  } else {
    ...
    storageEnabled = true;
    console.log('ImageKit berhasil diinisialisasi dengan endpoint:', process.env.IMAGEKIT_URL_ENDPOINT);
  }
} catch (error) {
...
const uploadProductImageToStorage = async (imageBase64, storeId, productUuid, productName) => {
  try {
    if (!storageEnabled) {
      console.warn('Percobaan upload foto produk tetapi Storage tidak diaktifkan');
...
const uploadProductVariantImageToStorage = async (imageBase64, storeId, productUuid, variantId, variantName) => {
  try {
    if (!storageEnabled) {
      console.warn('Percobaan upload foto variant produk tetapi Storage tidak diaktifkan');
      throw new Error('Storage tidak diaktifkan');
    }
    ...
    const cleanVariantName = variantName.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase();
    ...
    const uploadResponse = await imagekit.upload({
      ...
      fileName: fileName,
      folder: folderPath,
      tags: ['variant', `store_${storeId}`, `product_${productUuid}`, `variant_${variantId}`],
      useUniqueFileName: true,
      transformation: {
...
const uploadStoreLogoToStorage = async (logoBase64, storeId, storeUuid, storeName) => {
  try {
    if (!storageEnabled) {
      console.warn('Percobaan upload logo toko tetapi Storage tidak diaktifkan');
      throw new Error('Storage tidak diaktifkan');
    }
...
const uploadStoreStampToStorage = async (stampBase64, storeId, storeUuid, storeName) => {
  try {
    if (!storageEnabled) {
      console.warn('Percobaan upload stamp toko tetapi Storage tidak diaktifkan');
      throw new Error('Storage tidak diaktifkan');
    }
...
const uploadPostThumbnailToStorage = async (thumbnailBase64, postId, postSlug, authorId) => {
  try {
    if (!storageEnabled) {
      console.warn('Percobaan upload thumbnail post tetapi Storage tidak diaktifkan');
      throw new Error('Storage tidak diaktifkan');
    }
...
// Upload dokumen untuk order/invoice (jika diperlukan untuk attachment)
const uploadOrderDocumentToStorage = async (documentBase64, orderId, orderUuid, storeId, documentType = 'invoice') => {
  try {
    if (!storageEnabled) {
      console.warn('Percobaan upload dokumen order tetapi Storage tidak diaktifkan');
      throw new Error('Storage tidak diaktifkan');
    }
    ...
    const uploadResponse = await imagekit.upload({
      ...
      fileName: fileName,
      folder: folderPath,
      tags: [documentType, `order_${orderId}`, `store_${storeId}`, `uuid_${orderUuid}`],
      useUniqueFileName: true
    });
...
const uploadCustomerPhotoToStorage = async (photoBase64, customerId, storeId, customerName) => {
  try {
    if (!storageEnabled) {
      console.warn('Percobaan upload foto customer tetapi Storage tidak diaktifkan');
      throw new Error('Storage tidak diaktifkan');
    }
    ...
    const cleanCustomerName = customerName.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase();
...
// Fungsi hapus file dari ImageKit
async function deleteFileFromStorage(imageUrl) {
  try {
    ...
    if (imageUrl.includes('ik.imagekit.io')) {
      // Ambil fileId dari URL ImageKit
      const urlParts = imageUrl.split('/');
      const fileName = urlParts[urlParts.length - 1];
      
      // Cari file berdasarkan nama untuk mendapatkan fileId
...
// Fungsi untuk mendapatkan URL dengan transformasi
const getTransformedUrl = (imageUrl, transformations) => {
  if (!storageEnabled || !imageUrl) {
    return imageUrl;
  }
...
// Fungsi untuk list files berdasarkan tags
const listFilesByTags = async (tags, limit = 100, skip = 0) => {
  try {
    if (!storageEnabled) {
      console.warn('Storage tidak diaktifkan');
      return [];
    }
...
async function testStorageConnection() {
  ...
  try {
    // Test dengan list files (limit 1 untuk cek koneksi)
    await imagekit.listFiles({ limit: 1 });
    
    console.log('Tes koneksi ImageKit: BERHASIL');
    return true;
  } catch (error) {
    console.error('Tes koneksi ImageKit: GAGAL', error);
    return false;
  }
}
...
module.exports = {
  ...
  uploadProductVariantImageToStorage,
  uploadStoreLogoToStorage,
  uploadStoreStampToStorage,
  uploadPostThumbnailToStorage,
  uploadOrderDocumentToStorage,
  ...
  uploadPhotoToStorage: uploadOrderDocumentToStorage,
